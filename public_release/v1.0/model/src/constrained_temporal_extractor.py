...
DEFAULT_PRESENCE = ROOT / "outputs" / "event_extraction_refined_20260710" / "temporal_presence_transformer" / "test_predictions.csv"
DEFAULT_OUTPUT = ROOT / "outputs" / "event_extraction_refined_20260710" / "constrained_temporal_pipeline"
...
MONTH_DAY_RANGE_RE = re.compile(
    rf"\b(?P<month>{MONTH_PATTERN})\.?\s*(?P<start_day>\d{{1,2}})\s*(?:st|nd|rd|th)?\s*(?:-|–|—|to|through)\s*(?P<end_day>\d{{1,2}})\s*(?:st|nd|rd|th)?(?:\s*,?\s*(?P<year>20\d{{2}}))?\b",
    re.I,
)
...
BARE_CUED_TIME_RE = re.compile(r"\b(?:at|by)\s+(?P<hour>\d{1,2})(?!\s*[:/\d])\b", re.I)
DURATION_RE = re.compile(
    r"\b(?:(?P<hours>\d+(?:\.\d+)?)\s*(?:hours?|hrs?)|(?P<hours_word>one|two|three|four|five|six|seven|eight)\s+hours?|(?P<minutes>\d{1,3})\s*(?:minutes?|mins?))\b",
    re.I,
)
...
def posted_at_toronto(value: Any) -> datetime:
    ...
    try:
        ...
        # Reviewed spreadsheet timestamps are already Toronto-local wall time.
        # Only timezone-aware inputs require conversion. Treating a naive ISO
        # timestamp as UTC shifts every notice by four or five hours.
...
def date_mentions(text: str, posted: datetime) -> list[DateMention]:
    ...
    for match in MONTH_DAY_RANGE_RE.finditer(text):
        month = month_number(match.group("month"))
        year = int(match.group("year") or posted.year)
        value = infer_year(month, int(match.group("end_day")), posted, year if match.group("year") else None)
        if value:
            mentions.append(DateMention(value, match.start("end_day"), match.end("end_day"), 4, match.group("end_day")))
...
def single_times(text: str, excluded: Iterable[tuple[int, int]] = ()) -> list[tuple[int, int, int]]:
    ...
    occupied = [(start, end) for _, start, end in output]
    for match in BARE_CUED_TIME_RE.finditer(text):
        if any(match.start() < end and match.end() > start for start, end in [*excluded, *occupied]):
            continue
        hour = int(match.group("hour"))
        if hour > 23:
            continue
        marker = "pm" if hour <= 7 or hour == 12 else "am"
        output.append((_to_minutes(hour, 0, marker), match.start(), match.end()))
...
def extract_endpoints(row: dict[str, Any], has_start: bool, has_end: bool) -> tuple[str, str]:
    ...
    start = end = ""
    event_dates = date_mentions(candidate.segment, posted) if role == "event" else []
    event_start_date = event_dates[0].value if event_dates else date
    event_end_date = event_dates[-1].value if event_dates else date
    if candidate.time_range:
        ...
        if override:
            try:
                ...
                pass
        if has_start:
            start = format_datetime(event_start_date, start_minutes, False)
        if has_end:
            end = format_datetime(event_end_date, end_minutes, False)
        return start, end
    ...
    if role == "notice":
        ...
        if len(values) >= 2:
            start = format_datetime(event_start_date, values[0], False)
            end = format_datetime(event_end_date, values[1], False)
        elif values:
            start = format_datetime(event_start_date, values[0], False)
            duration = DURATION_RE.search(candidate.segment)
            if duration:
                duration_minutes = (
                    round(float(duration.group("hours")) * 60)
                    if duration.group("hours")
                    else WORD_NUMBERS[duration.group("hours_word").lower()] * 60
                    if duration.group("hours_word")
                    else int(duration.group("minutes"))
                )
                end = format_datetime(event_end_date, values[0] + duration_minutes, False)
            else:
                end = format_datetime(event_end_date, values[0], False)
        else:
            start = format_datetime(event_start_date, 0, True)
            end = format_datetime(event_end_date, 23 * 60 + 59, True)
    return start, end
...
def evaluate_pipeline(frame: pd.DataFrame, presence: pd.DataFrame, output_csv: Path) -> dict[str, Any]:
    # Evaluation is optional; keep sklearn out of the extraction runtime so the
    # temporal parser can be reused by lightweight data-preparation scripts.
    ...
    if "predicted_classification" in presence.columns:
        presence_columns.append("predicted_classification")
    merged = frame.merge(
        presence[presence_columns],
        on="reviewed_row_id",
        how="inner",
        validate="one_to_one",
    )
    ...
    for _, row in merged.iterrows():
        ...
        semantic_prediction = clean(row.get("predicted_classification")).lower()
        prediction = (
            semantic_prediction
            if semantic_prediction in LABELS
            else derive_classification(start_time, end_time)
        )
        records.append(
            {
    ...
    metrics.update(
        {
            ...
            "classification_rule": (
                "Endpoint topology constrains event/deadline states; when the "
                "presence file supplies a validation-calibrated semantic notice "
                "override, that reviewed exception is retained."
            ),
        }
    )
