...
DEFAULT_OUTPUT = MODEL_ROOT / "training" / "constrained_temporal_model"
...
def main() -> None:
                ...
                for start_threshold in threshold_values:
                    for end_threshold in threshold_values:
                        ...
                        key = (
                            ...
                            float(np.mean((validation_true_start == predicted_start) & (validation_true_end == predicted_end))),
                            float(np.mean(validation_true_start == predicted_start)),
                            float(np.mean(validation_true_end == predicted_end)),
                        )
    ...
    candidates.sort(
        key=lambda item: (
            ...
            item["validation"]["classification"]["accuracy"],
    ...
    final_metrics = {
        ...
        "selection": {
            "metric": "validation macro F1, then validation accuracy and joint presence accuracy",
            "feature_kind": kind,
            "C": float(selected["C"]),
