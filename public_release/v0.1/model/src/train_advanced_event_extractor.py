#!/usr/bin/env python3
"""Fine-tune a ModernBERT BIO tagger for the reviewed Canvas event dataset.
...
DEFAULT_OUTPUT = MODEL_ROOT / "training" / "span_model"
...
def build_gold_spans(
    ...
    if exact_title:
        ...
        fuzzy = fuzzy_word_span(text, title, 0)
        if fuzzy:
            spans.append((*fuzzy, "TITLE"))
    ...
    for location in locations:
        ...
        if compact:
            ...
            if match:
                ...
                continue
        fuzzy = fuzzy_word_span(text, location, body_start, threshold=0.60)
        if fuzzy:
            spans.append((*fuzzy, "LOCATION"))
...
def structured_prediction(
    ...
    if not url_values:
        # The deterministic URL layer is deliberately high recall; the neural
        # tagger still decides spans when it has a confident positive.
        for record in example.url_records:
            if record["url"] not in url_values and not ACTIONABLE_URL_EXCLUDE.search(record["url"]):
                url_values.append(record["url"])
    ...
    if not location and span_gate_classification != "not related":
        location = fallback_location(clean_text(example.row.get("body_text")))
...
def main() -> None:
    ...
    if any(row["row_origin"] != "reviewed_real" for row in validation_rows + test_rows):
        raise RuntimeError("Synthetic rows leaked into validation/test")
    ...
    assert not (train_groups & validation_groups or train_groups & test_groups or validation_groups & test_groups)
    ...
    validation_metrics, validation_structured, _ = evaluate_model(
        model, validation_dataset, collator, device, args.batch_size
    )
    test_metrics, test_structured, _ = evaluate_model(
    ...
    metrics = {
        ...
        "validation": validation_metrics,
        "test": test_metrics,
        "target_definition": "strict normalized exact-match accuracy >= 0.90 for each spreadsheet target; token F1 is reported separately",
